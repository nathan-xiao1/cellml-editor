  ...
  org.mathdox.parsing.ParserGenerator = $extend(Object, {
    ...
    transform : function(operand, transform) {
      ...
      return function(context, index, result, continuation) {
	    ...
	    var transformed;
            if (context.backward) {
              transformed = transform(newresult.slice(0, newresult.length - result.length));
              if (transformed !== undefined && transformed !== null) {
                continuation(newindex,[transformed].concat(result));
              }
            }
            else {
              transformed = transform(newresult.slice(result.length));
              if (transformed !== undefined && transformed !== null) {
                continuation(newindex, result.concat([transformed]));
              }
            }
          }
        );
