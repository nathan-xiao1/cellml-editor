...
$main(function(){
  ...
  org.mathdox.formulaeditor.semantics.List1List =
    $extend(org.mathdox.formulaeditor.semantics.MultaryListOperation, {
      ...
      bracketedRule : "listBracesWithArguments",
      bracketedRuleArgs : "listBracesWithArguments",
  ...
  org.mathdox.formulaeditor.parsing.expression.ExpressionContextParser.addFunction( 
    function(context) { return {
      ...
      listBraces:
        pG.transform(
          pG.rule("listBracesWithArguments"),
          function (result) {
            var list = new semantics.List1List();

            if (result[0] instanceof Array) {
              list.initialize.apply(list, result[0]); 
            } else {
              list.initialize.apply(list, result); 
            }

            return list;
          }
        ),
      listBracesWithArguments: pG.never
    }; }
  );
});
