...
$main(function(){
  ...
  org.mathdox.formulaeditor.semantics.Power =
	...
	if (sem.hasExplicitBrackets()) {
          ...
          brackets = false;
        } else if (sem instanceof semantics.Keyword) { // single keyword
          brackets = false;
        } else if (sem instanceof semantics.Variable) { // single variable
          brackets = false;
      ...
      onkeypress : function(event, editor) {
        ...
        if (!event.altKey && !event.ctrlKey) {
          ...
          if (String.fromCharCode(event.charCode) == "^") {
            ...
            // add empty box if nothing is before the superscript
            if (index == 0) {
              this.insert(index); 
