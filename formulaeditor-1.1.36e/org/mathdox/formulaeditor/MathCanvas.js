...
$main(function(){
  ...
  org.mathdox.formulaeditor.MathCanvas = $extend(Object, {
    ...
    getSymbolData : function(symbol, typeface, fontSizeModifier) {
      ...
      } else if (symbol == 'μ') { // U+03BC Greek small letter mu
        symbolData = this.getSymbolDataByPosition("mμ", fontSizeModifier);
      ...
      var marginData = [ 
        ...
        //U+204E low asterisk, for (modal logic: common knowledge (square with asterisk)
        { margin: 5, symbols: [ '⁎' ] }
      ];

      var m;

      for (m=0; m<marginData.length; m++) {
        if (marginData[m].symbols.indexOf(symbol)!= -1) {
          symbolData = this.extendObject(symbolData, { 
              margin: marginData[m].margin
          });
        }
      }
  ...
  org.mathdox.formulaeditor.MathCanvas.specialSymbols = {
    ...
    // U+2294 square cup
    // U+2293 square cap
    // U+25FB white medium square (modal logic: always, or necessarily)
    '◻': [ '⊔', '⊓' ],
    // box operator with asterix
